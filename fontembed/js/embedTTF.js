...
(function(window){
  ...
  Font.prototype.loadFont = function () {
    ...
    // If postBody defined, use POST instead of GET
    if(font.postBody){
      xhr.open('POST', font.url, true);
...
function load_TTF(classname, fid) {
  var elements = document.getElementsByClassName(classname);
  var nonUnique = '';
  for(var i=0;i<elements.length;++i){
    nonUnique = nonUnique + elements[i].textContent;
  }
  var uniqueTextArr = nonUnique.replace(/\s/g, "").split('').filter(function(item, i, ar){ return ar.indexOf(item) === i; });
  // Load font
  var font = new Font();
  // set up the onload handler
  font.onload = function() {
    // create a paragraph of text, styled with this font
    for (var i=0;i<elements.length;++i){
      elements[i].style.fontFamily = "'" + font.fontFamily + "'";
  ...
  font.src = "http://fontwalker.iis.sinica.edu.tw/include/getFontByWords.jsp"+"?fid="+fid+"&words="+uniqueTextArr.toString();
}